...
def preprocess_folder(folder_path, backup=True):
    """
    Preprocess semua file txt di folder dan simpan kembali ke file yang sama.
    ...
    if total_files == 0:
        print(f"Tidak ada file txt yang ditemukan di folder {folder_path}.")
        return False
...
if __name__ == "__main__":
    # Preprocess BIND folder
    bind_success = preprocess_folder("BIND")
    
    # Preprocess english texts folder
    english_success = preprocess_folder("english texts")
    
    if bind_success or english_success:
        print("\nPreprocessing telah selesai untuk semua folder!")
    else:
        print("\nTidak ada folder yang berhasil diproses. Periksa nama folder dan pastikan folder tersebut berisi file txt.")